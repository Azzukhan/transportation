...
export const updateContactRequestStatus = async (
  requestId: number,
  status: string,
): Promise<ContactRequestApi> => {
  const response = await apiClient.patch<ContactRequestApi>(
    `/public/contact-requests/${requestId}`,
    { status },
  );
  return response.data;
};
...
export const updateQuoteRequestStatus = async (
  ...
  const response = await apiClient.patch<QuoteRequestApi>(
    `/public/quote-requests/${requestId}`,
    { status },
  );
  return response.data;
};
